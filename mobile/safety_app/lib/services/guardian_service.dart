...
// }
// ===================================================================
// FIXED: guardian_service.dart - With Debug Logging
// ===================================================================
// lib/services/guardian_service.dart
...
class GuardianService {
  ...
  Future<List<DependentModel>> getMyDependents() async {
    try {
      ...
      final List<DependentModel> dependents = (response.data as List).map((
        json,
      ) {
        // 🐛 DEBUG: Print each JSON object before parsing
        print('📋 Parsing dependent JSON:');
        print('   dependent_name: ${json['dependent_name']}');
        print('   guardian_type: ${json['guardian_type']}');
        print('   is_primary: ${json['is_primary']}');
        print('   relation: ${json['relation']}');

        final model = DependentModel.fromJson(json);

        // 🐛 DEBUG: Print parsed model
        print('✅ Parsed to model:');
        print('   dependentName: ${model.dependentName}');
        print('   guardianType: ${model.guardianType}');
        print('   isPrimary: ${model.isPrimary}');
        print('   isPrimaryGuardian: ${model.isPrimaryGuardian}');
        print('   isCollaborator: ${model.isCollaborator}');
        print('---------------------------------------------------');
      ...
      print('═══════════════════════════════════════════════════');
      print('✅ SUMMARY: Fetched ${dependents.length} dependents');
      for (var dep in dependents) {
        print(
          '   - ${dep.dependentName}: ${dep.guardianType} (isPrimaryGuardian: ${dep.isPrimaryGuardian})',
        );
      }
      print('═══════════════════════════════════════════════════');
  ...
  Future<List<Map<String, dynamic>>> getDependentCollaborators(
    int dependentId,
  ) async {
    try {
      final response = await _dioClient.get(
